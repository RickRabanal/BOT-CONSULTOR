from pynput.keyboard import Key,Controller as ControllerK#key
...
class key(object):
    ...
    def Arrow(self,key="d",num=1,time=0.3):
        select={"w":lambda:self.__Buttons(Key.up),"a":lambda:self.__Buttons(Key.left),"s":lambda:self.__Buttons(Key.down),"d":lambda:self.__Buttons(Key.right),
                't':lambda:self.__Buttons(Key.tab),'st':lambda:pya.hotkey('shift','tab')}#t:tab,st:shift+tab
        for i in range(num):
            select[key]()
            pya.sleep(time)
...
class Excel(object):
    ...
    def Xlsx2Dict(self,excel='', sheet=1,read_only=False,head_filter=True,head_row=1,datetostr=True):
        ...
        self.__Sheet=_Sheet
        Header_f, Listado = [n.value for n in _Sheet[head_row] if n.value not in (None,'None','',' ')], []#_Sheet[1] : es la primera fila de la hoja
        maximo=len(Header_f)
        ...
        if head_filter:
            ...
            Header=Header_f
        self.Header=Header
        for idx,row in enumerate(_Sheet.iter_rows(),start=1):
            if idx<=head_row:continue
        ...
        if read_only:
            ...
            return Listado
        print(Header)
        Sheet =lambda idx,name:_Sheet.cell(row=1+head_row+idx, column=Header.index(name)+1)## Sheet(0,'QUIEBRE').value='hola'
